...
Cypress.Commands.add("addToCart", (idx) => {
  ...
  cy.get(Cypress.env("TEST_WP_THEME_PRODUCT_QUANTITY_SELECTOR"))
    .clear()
    .type(
      getRandomInt(
        Cypress.env("TEST_PRODUCT_QUANTITY_FROM"),
        Cypress.env("TEST_PRODUCT_QUANTITY_TO")
      )
    );
...
Cypress.Commands.add("fillCheckout", (b) => {
  ...
  cy.get("#select2-billing_country-results .select2-results__option")
    .contains(b.country)
    .should("exist");
  ...
  cy.contains("#select2-billing_company-results .select2-results__option", companyIdRegex).should(
    "exist"
  );
