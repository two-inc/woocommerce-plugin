...
Cypress.Commands.add('setGlobalVars', (name, value) => {
    if (value) {
...
Cypress.Commands.add('getGlobalVars', (name) => {
    if (name && globalVars[name]) {
        return globalVars[name]
    }
})
...
Cypress.Commands.add('adminGoToOrders', () => {
    cy.contains('.wp-submenu a[aria-current="page"]', 'Orders').should('exist')
    cy.contains('.wp-submenu a[aria-current="page"]', 'Orders').click()
    cy.contains('.wp-heading-inline', 'Orders').should('exist')
})
