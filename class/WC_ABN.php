...
if (!class_exists('WC_ABN')) {
    class WC_ABN extends WC_Payment_Gateway
    {
            ...
            if (!$days_on_invoice_last_checked_on || ($days_on_invoice_last_checked_on + 3600) <= time()) {
                ...
                if ($merchant_id && $this->get_option('api_key')) {
                    ...
                    if (!is_wp_error($response)) {
                        ...
                        if (!$abn_err && $response && $response['body']) {
                            $body = json_decode($response['body'], true);
                            if ($body['due_in_days']) {
                                $days_on_invoice = $body['due_in_days'];
                            } else {
                                // If DB has null value, also default to 14 days
                                $days_on_invoice = 14;
                            }
                        }
                    }
        ...
        public function verify_api_key($api_key = null)
        {
            ...
            if (!$api_key_to_use || !$this->get_abn_checkout_host()) {
                return;
            }
        ...
        public function abn_admin_styles_scripts()
        {
            ...
            wp_localize_script('abn.admin', 'abn_admin', [
                'nonce' => wp_create_nonce('abn_admin_nonce'),
            ...
            if ('mark_completed' === $doaction) {
                ...
                $redirect = add_query_arg(
                    array(
                        ...
                        'abn_success' => implode(",", $success),
                        'abn_failure' => implode(",", $failure),
                    ),
                    $redirect
                );
        ...
        public static function on_order_bulk_edit_notices()
        {
            ...
            $failure_order_ids = [];
            if (isset($_REQUEST['abn_failure']) && $_REQUEST['abn_failure']) {
                $failure_order_ids = explode(",", $_REQUEST['abn_failure']);
            }
            $success_order_ids = [];
            if (isset($_REQUEST['abn_success']) && $_REQUEST['abn_success']) {
                $success_order_ids = explode(",", $_REQUEST['abn_success']);
            }
        ...
        public function list_out_of_sync_order_ids($start_time, $end_time)
        {
            ...
            $select_no_abn_state = $wpdb->prepare(
            ...
            $results = $wpdb->get_results($select_no_abn_state);
            foreach ($results as $row) {
                $ids[$row->id] = [
                    'abn_state' => null,
                    'abn_id' => $row->abn_oid,
        ...
        public function process_payment($order_id)
        {
            ...
            if (has_filter('abn_payment_reference_message')) {
                $payment_reference_message = apply_filters('abn_payment_reference_message', $order_id);
                $order->update_meta_data('_payment_reference_message', $payment_reference_message);
            }
            $payment_reference_ocr = '';
            if (has_filter('abn_payment_reference_ocr')) {
                $payment_reference_ocr = apply_filters('abn_payment_reference_ocr', $order_id);
                $order->update_meta_data('_payment_reference_ocr', $payment_reference_ocr);
            }
            $payment_reference = '';
            $payment_reference_type = '';
            if (has_filter('abn_payment_reference')) {
                $payment_reference = apply_filters('abn_payment_reference', $order_id);
        ...
        public function init_form_fields()
        {
            $abn_form_fields = [
                ...
                'enable_company_search_for_others' => [
                    'title'       => __('Enable company name search for other payment options', 'abn-payment-gateway'),
                    'description' => __('Enables searching for company name even when other payment options are selected. Requires the option above to be checked.', 'abn-payment-gateway'),
                    'desc_tip'    => true,
                    'label'       => ' ',
                    'type'        => 'checkbox',
                    'default'     => 'yes'
                ...
                'section_debug' => [
                    'type'  => 'title',
                    'title' => __('Debug Options', 'abn-payment-gateway'),
                ],
                'enable_api_logging' => [
                    'title'       => __('Enable API Logging', 'abn-payment-gateway'),
                    'label'       => __('Log API requests and responses', 'abn-payment-gateway'),
                    'type'        => 'checkbox',
                    'description' => sprintf(
                        __('If enabled, all API interactions will be logged. This can be useful for debugging. You can view the logs <a href="%s">here</a>.', 'abn-payment-gateway'),
                        admin_url('admin.php?page=wc-status&tab=logs&source=abn-payment-gateway')
                    ),
                    'default'     => 'yes',
                ],
            ];
            ...
            $this->form_fields = apply_filters('wc_abn_form_fields', $abn_form_fields);
        }
            ...
            ?> />
                        ...
                        <?php if ($this->get_option('api_key') && $merchant_id = $this->get_merchant_id()) : ?>
                            <div style="margin-top: 8px; color: #666; font-size: 13px;">
                                <strong><?php _e('Merchant ID:', 'abn-payment-gateway'); ?></strong> <?php echo esc_html($merchant_id); ?>
                            </div>
                        <?php else: ?>
                            <div style="margin-top: 8px; color: #666; font-size: 13px;">
                                <strong><?php printf(__('Don\'t have an API key? Get one by signing up <a href=\'%s\'>here</a>.', 'abn-payment-gateway'), $this::MERCHANT_SIGNUP_URL); ?></strong>
                            </div>
                        <?php endif; ?>
...
<?php
        ...
        private function make_request($endpoint, $payload = [], $method = 'POST', $params = array(), $api_key_override = null)
        {
            ...
            $response = wp_remote_request(sprintf('%s%s?%s', $this->get_abn_checkout_host(), $endpoint, http_build_query($params)), [
                'method' => $method,
                'headers' => $headers,
                'timeout' => 30,
        ...
        public function abn_account_init_notice()
        {
            ...
            if (
                ...
                $_GET['tab'] === 'checkout' &&
                $_GET['section'] === 'woocommerce-gateway-abn'
            ) {
                return;
            }
            ...
            $benefits = sprintf(__('%s credit approves 90%% of business buyers, pays you upfront and minimise your risk. To offer %s in your checkout, you need to signup. It is quick, easy and gives you immediate access to the %s Merchant Portal.', 'abn-payment-gateway'), self::PRODUCT_NAME, self::PRODUCT_NAME, self::PRODUCT_NAME);
            $setup_account = __('Set up my account', 'abn-payment-gateway');
            $setup_url = admin_url('admin.php?page=wc-settings&tab=checkout&section=woocommerce-gateway-abn');
            echo '
        ...
        public function process_admin_options()
        {
            ...
            $api_key_field = 'woocommerce_woocommerce-gateway-abn_api_key';
            ...
            if ($api_key_in_post && $api_key) {
                ...
                if (isset($result['body']) && isset($result['code']) && $result['code'] == 200) {
                    ...
                    WC_Admin_Settings::add_error(__('Failed to verify API key.', 'abn-payment-gateway'));
                }
            }
