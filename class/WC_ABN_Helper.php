...
if (!class_exists('WC_ABN_Helper')) {
    class WC_ABN_Helper
    {
                ...
                if ($response['body']) {
                    ...
                    if (!is_string($body) && isset($body['error_json']) && is_array($body['error_json'])) {
                        ...
                        foreach ($body['error_json'] as $err) {
                            if ($err) {
                                $display_msg = WC_ABN_Helper::get_msg_from_err($err);
                                if ($display_msg) {
        ...
        public static function get_msg_from_err($err)
        {
            ...
            $loc_str = json_encode(WC_ABN_Helper::utf8ize($err['loc']));
            $msg_str = $err['msg'];
            ...
            if (strpos($loc_str, '["invoice_details","invoice_emails"') === 0) {
                return sprintf($generic_err_template, __('Invoice email address', 'abn-payment-gateway'));
            }
        }
        ...
        public static function compose_abn_order(
            ...
            if (has_filter('abn_order_create')) {
                $req_body = apply_filters('abn_order_create', $req_body);
            }
        ...
        public static function compose_abn_edit_order(
            ...
            if (has_filter('abn_order_edit')) {
                $req_body = apply_filters('abn_order_edit', $req_body);
            }
